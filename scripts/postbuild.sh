...
image_marker="$evidence_dir/image.ok"
...
syft "$image_ref" --scope all-layers -q \
  ...
  -o "cyclonedx-json=$evidence_dir/sbom.cyclonedx.json"
...
trivy image \
  ...
  --exit-code 1 \
  --format json \
  --output "$evidence_dir/trivy-image.json" \
  "$image_ref"
...
grype "$image_ref" \
  --fail-on high \
  --output json \
  --file "$evidence_dir/grype-image.json"
...
if [[ "$runtime_user" != "65532:65532" || "$entrypoint" != '["/dar-download"]' ]]; then
  printf 'unexpected image user or entrypoint: %s %s\n' "$runtime_user" "$entrypoint" >&2
  exit 1
fi
inspect_container="dar-download-inspect-$$"
rootfs_archive="$evidence_dir/image-rootfs.tar"
binary_path="$evidence_dir/dar-download.binary"
...
tar -tf "$rootfs_archive" >"$evidence_dir/image-files.txt"
tar -xOf "$rootfs_archive" dar-download >"$binary_path"
...
trap - EXIT INT TERM
if grep -Eq '(^|/)(bin/(ba)?sh|usr/bin/(apt|apt-get|dpkg)|sbin/apk)$' "$evidence_dir/image-files.txt"; then
